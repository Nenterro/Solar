...
test_ids = [
    ...
    "bse_battery_voltage_to_turnoff",
    "bse_battery_voltage_to_turnon",
    "bse_battery_voltage_turnoff",
    "bse_battery_voltage_turnon",
    "bse_battery_voltage_turn_off",
    "bse_battery_voltage_turn_on"
]

for inv in inverters:
    print("\n==================================")
    print(f"QUERYING DESS CTRL FOR {inv['id']} ({inv['sn']})")
    print("==================================")
    for ctrl_id in test_ids:
        salt = str(int(time.time() * 1000))
        ...
        try:
            ...
            if r.get("err") == 0:
                print(f"  {ctrl_id:35s} -> {r.get('dat')}")
            else:
                print(f"  {ctrl_id:35s} -> ERR: {r.get('desc')}")
