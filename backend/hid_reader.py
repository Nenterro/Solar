...
class HIDInverterReader:
    ...
    def poll_all_inverters(self) -> Dict[str, Dict[str, Any]]:
            ...
            if node_path and os.path.exists(node_path):
                ...
                if qpigs_resp:
                    try:
                        ...
                        logger.error(f"QPIGS parse error for {inv_id} at {node_path}: {e}")
        ...
        # Log 1-minute high-resolution snapshot to DB
        t = time.time()
        if t - self.last_db_log_time >= 60:
            db.log_telemetry_snapshot(self.readings_cache)
            self.last_db_log_time = t
